(function () {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setProgress(40);
      if (payment && payment.skipCheckout && payment.reference) {
        setInfo("Payment already exists for this plan. Resuming generation...");
        setProgress(70);
        var completedReuse = await completePayment(payment.reference);
        state.result = completedReuse;
        if (planPreview) planPreview.textContent = String(completedReuse.planText || "");
        if (openDashboardLink && completedReuse.dashboardUrl) {
          openDashboardLink.href = String(completedReuse.dashboardUrl);
        }
        if (resultWrap) resultWrap.classList.remove("hidden");

        var reusedStatus = String(completedReuse.verificationStatus || "awaiting_verification").toLowerCase();
        var reusedAwaiting = reusedStatus !== "verified";
        if (downloadDocxBtn) downloadDocxBtn.disabled = reusedAwaiting;
        if (downloadDocxBtn) downloadDocxBtn.textContent = reusedAwaiting ? "Download Locked (Awaiting Verification)" : "Download DOCX";
        if (completedReuse && completedReuse.generationQueued) {
          setInfo("Payment already confirmed. Plan generation is in progress in the background. Check your dashboard shortly.");
        } else {
          setInfo(
            reusedAwaiting
      ...
      openPaystackInline(
        ...
        async function (reference) {
          try {
            ...
            if (completed && completed.generationQueued) {
              setInfo("Payment confirmed. Plan generation is in progress in the background. Check your dashboard shortly.");
            } else {
              setInfo(
                awaiting
                  ? "Business plan generated by AI and sent for expert verification. Check your dashboard for status updates."
