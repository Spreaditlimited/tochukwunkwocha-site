(function () {
  ...
  async function retryDomainRegistration(orderUuid) {
    return api("/.netlify/functions/domain-registration-retry", {
  ...
  function bindDnsEvents() {
    ...
    listEl.addEventListener("click", async function (event) {
      ...
      const retryBtn = event.target.closest("[data-domain-retry]");
      if (retryBtn) {
        const orderUuid = String(retryBtn.getAttribute("data-domain-retry") || "").trim();
        if (!orderUuid) return;
        try {
          ...
          if (metaEl) {
            metaEl.textContent = `Retrying registration for order ${orderUuid}...`;
          }
          const json = await retryDomainRegistration(orderUuid);
          if (metaEl) {
            if (json && String(json.status || "").toLowerCase() === "registered") {
              metaEl.textContent = "Domain registration completed successfully.";
            } else {
              metaEl.textContent = (json && json.message) || "Retry submitted. Registration is still processing.";
            }
          }
          await load();
          retryBtn.textContent = originalText || "Retry Registration";
        } catch (error) {
          retryBtn.disabled = false;
          if (metaEl) {
            metaEl.textContent = error.message || "Could not retry registration.";
          } else {
            alert(error.message || "Could not retry registration.");
          }
        }
        return;
      }
  ...
  async function load() {
    try {
      ...
      (function showDomainRegistrationState() {
        if (!metaEl) return;
        const query = new URLSearchParams(window.location.search || "");
        const state = String(query.get("domain") || "").trim().toLowerCase();
        const orderUuid = String(query.get("order") || "").trim();
        if (state === "payment_confirmed") {
          metaEl.textContent = orderUuid
            ? `Payment confirmed. We are now registering your domain (order ${orderUuid}).`
            : "Payment confirmed. We are now registering your domain.";
          return;
        }
        if (state === "registration_failed") {
          metaEl.textContent = "Payment succeeded, but domain registration is pending. Use Retry Registration on the order card.";
        }
      })();

      if (!domains.length) {
        if (listEl) {
          const pendingOrder = orders.find(function (o) {
            const status = String((o && o.status) || "").toLowerCase();
            return status === "registration_in_progress" || status === "registration_failed";
          });
          ...
          listEl.innerHTML = [
            ...
            hasPendingRegistrationOrder && pendingOrderUuid
              ? `<button type="button" data-domain-retry="${escapeAttr(
                  pendingOrderUuid
                )}" class="mt-4 inline-flex items-center rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-brand-500">Retry Registration</button>`
        ...
        ordersEl.innerHTML = orders
          .map(function (item) {
            ...
            return [
              ...
              canRetry
                ? `<div class="mt-3"><button type="button" data-domain-retry="${escapeAttr(
                    item.orderUuid || ""
                  )}" class="inline-flex items-center justify-center rounded-lg border border-brand-300 bg-brand-50 px-3 py-1.5 text-xs font-bold text-brand-700 hover:bg-brand-100">Retry Registration</button></div>`
                : "",
              "</article>",
            ].join("");
          })
