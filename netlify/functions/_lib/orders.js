...
const { resolveCourseBatch, normalizeBatchKey, ensureCourseBatchesTable } = require("./batch-store");
...
async function reconcileCoursePaystackOrders(pool, input) {
  ...
  const where = [
    ...
    "TRIM(provider_reference) <> ''",
  ];
  const params = [courseSlug];
  if (requestedBatchKey && requestedBatchKey !== "all") {
    where.push("batch_key = ?");
    params.push(requestedBatchKey);
  }
